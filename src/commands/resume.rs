...
pub async fn handle(ctx: Context, interaction: &CommandInteraction) {
    ...
    if !session_lock.paused {
        drop(session_lock);
        respond_command(&ctx, interaction, "Already playing").await;
        return;
    }
    ...
    if session_lock.current_module.is_some() {
        ...
        drop(session_lock);
        respond_command(&ctx, interaction, "No module is currently paused").await;
        return;
    }

    drop(session_lock);
    respond_command(&ctx, interaction, "Playback resumed").await;
}
